...
export class EmpregadosComponent implements OnInit {
  ...
  async listLocalData() {
    ...
    this.empregadosOff = this.empregadosService.getDataOffline().pipe(
...
export class CreateEmployeeModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.data) {
      ...
      this.levelSelected = this.empregado.nivel; //set the level in the form
    }
    this.levelSelect();
  }

  levelSelect() {
    this.empregado.nivel = this.levelSelected;
  }
